...
import psychlua.backend.DebugLuaText;
...
class HScript extends Iris
{
	...
	override public function set(key:String, value:Dynamic, allowOverride:Bool = true):Void
	{
		...
		{
			// Try to resolve via ClassResolver
			var resolvedClass = ClassResolver.resolveClass(key);
	...
	override function preset()
	{
		...
		// addHaxeLibrary for old mods - uses ClassResolver for path redirection
		// Example: addHaxeLibrary('PlayState') or addHaxeLibrary('Conductor', 'backend')
		//          Both work thanks to ClassResolver mapping old paths to new ones
		set('addHaxeLibrary', function(libName:String, ?libPackage:String = '')
		{
			try
			{
				...
				// Uses ClassResolver.resolveClass for backwards compatibility with old Psych paths
				var resolvedClass = ClassResolver.resolveClass(className);
	...
	public static function implement(funk:FunkinLua)
	{
		...
		funk.addLocalCallback("addHaxeLibrary", function(libName:String, ?libPackage:String = '')
		{
			...
			var className = str + libName;
			var c:Dynamic = ClassResolver.resolveClass(className);
	 ...
	 */
	public function getScriptedClass(name:String):psychlua.backend.ScriptedClass.ScriptClassHandler
	{
		@:privateAccess
		var v:Dynamic = interp.customClasses.get(name);
		if (v != null && (v is psychlua.backend.ScriptedClass.ScriptClassHandler))
			return cast v;
		return null;
	}
...
class CustomInterp extends crowplexus.hscript.Interp
{
	...
	override function get(o:Dynamic, field:String):Dynamic
	{
		...
		if ((o is psychlua.backend.ScriptedClass.IScriptCustomBehaviour))
			return cast(o, psychlua.backend.ScriptedClass.IScriptCustomBehaviour).hget(field);
	...
	override function set(o:Dynamic, field:String, value:Dynamic):Dynamic
	{
		...
		if ((o is psychlua.backend.ScriptedClass.IScriptCustomBehaviour))
			return cast(o, psychlua.backend.ScriptedClass.IScriptCustomBehaviour).hset(field, value);
